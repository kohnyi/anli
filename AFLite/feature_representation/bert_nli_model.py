...
class BertNLIModel(nn.Module):
    ...
    """
    def __init__(self,model_path=None,gpu=False,bert_type='bert-base',label_num=3,batch_size=8,reinit_num=0,freeze_layers=False):
    ...
    def forward(self, sent_pair_list, checkpoint=True, bs=None):
        ...
        for batch_idx in tqdm(range(0,len(sent_pair_list),bs), disable=no_prog_bar,desc='evaluate'):
            print("\rBatch [{}/{}]".format(batch_idx, len(sent_pair_list)), end=" ")
            cls_vecs, logits, probs = self.ff(sent_pair_list[batch_idx:batch_idx+bs],checkpoint)
            cls_vecs = cls_vecs.data.cpu().numpy()
            # probs = probs.data.cpu().numpy()
            if all_fv is None:
                all_fv = cls_vecs
                # all_probs = probs
            else:
                all_fv = np.append(all_fv, cls_vecs, axis=0)
        ...
        return all_fv, labels, all_probs
